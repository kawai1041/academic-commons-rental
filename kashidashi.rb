...
# 2015.07.17 貸出時間のデフォールト値を設定できるようにした。
#            sc.iniにrental_durartion:　で分数を設定する。
#                 0ならば貸出時刻が返却予定時刻になる。
#                 0以外の正の整数を入れると現在時刻＋指定分数が返却時刻になる。
#                 rental_durationを書かないと返却予定時刻を入力する必要あり。
# 2015.07.17 貸出日を貸出ごとに設定するようにした。
...
module Frm_form1
  ...
  def _form1_init
    ...
    if @ini.duration == nil or @ini.duration != 0
      ...
      addControl(VRStatic,'static4',"貸出時刻",256,96,100,24,1342177280)
    end
